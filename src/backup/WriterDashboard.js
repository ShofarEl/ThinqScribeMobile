...
const WriterDashboard = () => {
  ...
  const handleConfirmComplete = async () => {
    ...
    try {
      ...
      // Update local state immediately for better UX
      const completedAgreement = activeAgreements.find(a => a._id === selectedAgreement._id);
      if (completedAgreement) {
        // Remove from active agreements
        setActiveAgreements(prev => prev.filter(a => a._id !== selectedAgreement._id));
        
        // Add to completed agreements
        setCompletedAgreements(prev => [
          { ...completedAgreement, status: 'completed', completedAt: new Date() },
          ...prev
        ]);
