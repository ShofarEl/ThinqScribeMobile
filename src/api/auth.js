// ThinqScribe/src/api/auth.js
import client from './client';
import AsyncStorage from '@react-native-async-storage/async-storage';

export const storeAuthToken = async (token) => {
  console.log('💾 [AuthService] Storing token');
  await AsyncStorage.setItem('thinqscribe_auth_token', token);
};

export const getAuthToken = async () => {
  return await AsyncStorage.getItem('thinqscribe_auth_token');
};
...
export const logout = async () => {
  try {
    ...
    await clearAuthToken(); // Clear token even if backend logout fails
    throw error;
  }
};
